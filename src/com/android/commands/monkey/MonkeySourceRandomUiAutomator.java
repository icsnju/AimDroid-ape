/*
 * Copyright (C) 2020 Institute of Computer Software, Nanjing University, China
 *
 ...
 * monkey event queue
 */
public class MonkeySourceRandomUiAutomator implements MonkeyEventSource {
    ...
    public MonkeySourceRandomUiAutomator(Random random, List<ComponentName> MainApps,
    ...
    /**
     * Generate a tap event at the center of a given node.
     */
    protected void generateTapEventAtNodeRandom(AccessibilityNodeInfo root, Random random) {
        List<AccessibilityNodeInfo> clickable = new ArrayList<AccessibilityNodeInfo>();
        collectClickable(clickable, root);

        int count = clickable.size();
        if (count == 0) {
            //generate a random click event to avoid crash by no event
            generatePointerEvent(random, GESTURE_TAP);
            return;
        }

        AccessibilityNodeInfo node = clickable.get(random.nextInt(count));
    ...
    private void generateEvents() {
        ...
        if (info != null) {
            generateTapEventAtNodeRandom(info, mRandom);
            return;
        } else {
    ...
    public boolean validate() {
        ...
        return ret & adjustEventFactors() & mHandlerThread.isAlive();
    }
